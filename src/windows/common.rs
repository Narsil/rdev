...
pub unsafe fn get_code(lpdata: isize) -> u32 {
    let kb = *(lpdata as *const KBDLLHOOKSTRUCT);
    kb.vkCode
}
pub unsafe fn get_scan_code(lpdata: isize) -> u32 {
    let kb = *(lpdata as *const KBDLLHOOKSTRUCT);
    kb.scanCode
}
pub unsafe fn get_point(lpdata: isize) -> (i32, i32) {
    let mouse = *(lpdata as *const MSLLHOOKSTRUCT);
    (mouse.pt.x, mouse.pt.y)
}
/// https://docs.microsoft.com/en-us/previous-versions/windows/desktop/legacy/ms644986(v=vs.85)
pub unsafe fn get_delta(lpdata: isize) -> i32 {
    let mouse = *(lpdata as *const MSLLHOOKSTRUCT);
    (mouse.mouseData as i32 >> 16) as i32
}
pub unsafe fn get_button_code(lpdata: isize) -> i32 {
    let mouse = *(lpdata as *const MSLLHOOKSTRUCT);
    (mouse.mouseData as i32 >> 16) as i32
}

pub static mut LAST_CODE: u32 = 0;
pub static mut LAST_SCAN_CODE: u32 = 0;
pub static mut LAST_STATE: [u8; 256] = [0; 256];
pub static mut LAST_IS_DEAD: bool = false;

pub unsafe fn get_name(lpdata: isize) -> Option<String> {
...
pub unsafe fn convert(param: usize, lpdata: isize) -> Option<EventType> {
    match param {
